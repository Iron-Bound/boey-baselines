...
if __name__ == "__main__":
    ...
    ep_length = (
        2048 * 10 * 4
    )  # increased ep length for converging model, added early stop
    n_steps = int(
        5120 // cpu_multiplier
    )  # to maintain ~163_840 steps per training iteration
    sess_id = str(uuid.uuid4())[:8]
    sess_path = Path(f"session_{sess_id}_env8_lr3e-4_ent01_bs512_5120_81920_0.5vf")
    num_cpu = int(
        24 * cpu_multiplier
    )  # Also sets the number of episodes per training iteration
    env_config = {
        "headless": True,
    ...
    if exists(file_name + ".zip"):
        ...
        print(model.policy)
        print(
            f'Loaded model --- LR: {model.learning_rate} OptimizerLR: {model.policy.optimizer.param_groups[0]["lr"]}, ent_coef: {model.ent_coef}'
        )
        ...
        print(
            f'start training --- LR: {model.learning_rate} OptimizerLR: {model.policy.optimizer.param_groups[0]["lr"]}, ent_coef: {model.ent_coef}'
        )
