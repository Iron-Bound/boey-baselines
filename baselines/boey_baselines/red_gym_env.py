...
class RedGymEnvV3(Env):
    def __init__(self, config=None):
        ...
        self.randomize_first_ep_split_cnt = (
            0
            if "randomize_first_ep_split_cnt" not in config
            else config["randomize_first_ep_split_cnt"]
        )
        ...
        self.debug_buttons = (
            False if "debug_buttons" not in config else config["debug_buttons"]
        )
        self.restricted_start_menu = (
            False
            if "restricted_start_menu" not in config
            else config["restricted_start_menu"]
        )
        self.instance_id = (
            str(uuid.uuid4())[:8]
        ...
        self.save_state_dir = (
            None if "save_state_dir" not in config else config["save_state_dir"]
        )
        ...
        if self.debug_buttons:
            self.valid_actions.extend([WindowEvent.PASS])
        ...
        self.observation_space = spaces.Dict(
            {
                ...
                "vector": spaces.Box(
                    low=-1, high=1, shape=self.output_vector_shape, dtype=np.float32
                ),
                "map_ids": spaces.Box(low=0, high=255, shape=(1,), dtype=np.uint8),
                "item_ids": spaces.Box(low=0, high=255, shape=(20,), dtype=np.uint8),
                "item_quantity": spaces.Box(
                    low=-1, high=1, shape=(20, 1), dtype=np.float32
                ),
                "poke_ids": spaces.Box(low=0, high=255, shape=(12,), dtype=np.uint8),
                "poke_type_ids": spaces.Box(
                    low=0, high=255, shape=(12, 2), dtype=np.uint8
                ),
                "poke_move_ids": spaces.Box(
                    low=0, high=255, shape=(12, 4), dtype=np.uint8
                ),
                "poke_move_pps": spaces.Box(
                    low=-1, high=1, shape=(12, 4, 2), dtype=np.float32
                ),
                "poke_all": spaces.Box(
                    low=-1,
                    high=1,
                    shape=(12, self.n_pokemon_features),
                    dtype=np.float32,
                ),
                "event_ids": spaces.Box(low=0, high=2570, shape=(10,), dtype=np.int16),
                "event_step_since": spaces.Box(
                    low=-1, high=1, shape=(10, 1), dtype=np.float32
                ),
    ...
    def render(self, reduce_res=True, add_memory=True, update_mem=True):
        ...
        if reduce_res:
            game_pixels_render = game_pixels_render[
                :, :, 0
            ]  # should be 3x speed up for rendering
            game_pixels_render = (
    ...
    def step(self, action):
        ...
        if (
            not self.warmed_up
            and self.randomize_first_ep_split_cnt
            and self.step_count
            and self.step_count % (self.max_steps // self.randomize_first_ep_split_cnt)
            == 0
            and 1.0 / self.randomize_first_ep_split_cnt > np.random.rand()
        ):
            # not warmed up yet
            ...
            self.warmed_up = True
            print(
                f"randomly end episode at step {self.step_count} with randomize_first_ep_split_cnt: {self.randomize_first_ep_split_cnt}"
            )
    ...
    def update_past_events(self):
        if (
            self.past_events_string
            and self.past_events_string != self.all_events_string
        ):
            self.last_10_event_ids = np.roll(self.last_10_event_ids, 1, axis=0)
            self.last_10_event_ids[0] = [
                self.get_first_diff_index(
                    self.past_events_string, self.all_events_string
                ),
                self.step_count,
            ]
    ...
    def check_if_early_done(self):
        ...
        if self.early_stopping and self.step_count > 10241:
            ...
            if self.early_done:
                print(
                    f"es, step: {self.step_count}, r1: {self.past_rewards[0]}, r2: {self.past_rewards[-1]}"
                )
    ...
    def get_visited_pokecenter_reward(self):
        ...
        if (
            last_pokecenter_id != -1
            and last_pokecenter_id not in self.visited_pokecenter_list
        ):
    ...
    def get_pokemon_types(self, start_addr):
        return [
            self.fix_pokemon_type(self.read_m(start_addr + i)) + 1 for i in range(2)
        ]
    ...
    def get_all_pokemon_types_obs(self):
        ...
        if self.is_in_battle():
            ...
            if not self.is_wild_battle():
                ...
                wild_ptypes = self.get_pokemon_types(
                    0xCFEA
                )  # 2 ptypes only, add padding for remaining 5
        ...
        assert result.shape == (
            12,
            2,
        ), f"invalid ptypes shape: {result.shape}"  # set PYTHONOPTIMIZE=1 to disable assert
        return result
