...
class CustomCombinedExtractorV2(BaseFeaturesExtractor):
    ...
    def forward(self, observations: TensorDict) -> th.Tensor:
        ...
        embedded_poke_move_ids = self.poke_move_ids_embedding(
            observations["poke_move_ids"].to(th.int)
        )
        ...
        embedded_poke_type_ids = self.poke_type_ids_embedding(
            observations["poke_type_ids"].to(th.int)
        )
        ...
        pokemon_concat = th.cat(
            [poke_moves, poke_types, poke_ids, poke_stats], dim=-1
        )  # (12, 63)
        pokemon_features = self.poke_fc_relu(pokemon_concat)  # (12, 32)

        # Pokemon party head
        party_pokemon_features = pokemon_features[
            ..., :6, :
        ]  # (6, 32), ... for batch dim
        poke_party_head = self.poke_party_head(party_pokemon_features)  # (6, 32)
        poke_party_head = self.poke_party_head_max_pool(poke_party_head).squeeze(
            -2
        )  # (6, 32) -> (32, )

        # Pokemon opp head
        opp_pokemon_features = pokemon_features[
            ..., 6:, :
        ]  # (6, 32), ... for batch dim
        poke_opp_head = self.poke_opp_head(opp_pokemon_features)  # (6, 32)
        poke_opp_head = self.poke_opp_head_max_pool(poke_opp_head).squeeze(
            -2
        )  # (6, 32) -> (32, )

        # Items
        embedded_item_ids = self.item_ids_embedding(
            observations["item_ids"].to(th.int)
        )  # (20, 16)
        # item_quantity
        ...
        item_features = self.item_ids_max_pool(item_features).squeeze(
            -2
        )  # (20, 16) -> (16, )

        # Events
        embedded_event_ids = self.event_ids_embedding(
            observations["event_ids"].to(th.int)
        )
        # event_step_since
        event_step_since = observations["event_step_since"]  # (20, 1)
        event_concat = th.cat(
            [embedded_event_ids, event_step_since], dim=-1
        )  # (20, 17)
        event_features = self.event_ids_fc_relu(event_concat)
        event_features = self.event_ids_max_pool(event_features).squeeze(
            -2
        )  # (20, 16) -> (16, )
